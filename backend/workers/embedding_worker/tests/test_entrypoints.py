...
from embedding_worker import aws_lambda, config
...
def test_the_lambda_handler_runs_this_worker(lambda_db):
    ...
    assert (handler._queue_name, handler._service) == ("embedding_jobs", "embedding_worker")
    assert isinstance(handler._worker, Worker)
    assert handler._worker._queue is handler._queue
    assert handler._worker._queue_name == "embedding_jobs"
