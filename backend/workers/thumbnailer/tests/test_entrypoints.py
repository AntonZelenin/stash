...
from thumbnailer import aws_lambda, config
...
def test_the_lambda_handler_runs_this_worker(lambda_db):
    ...
    assert (handler._queue_name, handler._service) == ("thumbnail_jobs", "thumbnailer")
    assert isinstance(handler._worker, Worker)
    assert handler._worker._queue is handler._queue
    assert handler._worker._queue_name == "thumbnail_jobs"
