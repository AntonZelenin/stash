...
async def record_thumbnail(
    engine: AsyncEngine, item_id: UUID, *, thumbnail_key: str, analysis_job: ProcessingJob
) -> bool:
    ...
    `item_images` row) no longer exists. Re-recording the same key is a
    harmless no-op change; its job is a duplicate the analysis stage skips
    or runs idempotently."""
    async with engine.begin() as conn:
        result = await conn.execute(
