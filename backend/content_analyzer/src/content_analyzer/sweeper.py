...
class StaleItemSweeper:
    ...
    are marked `failed`. Re-publishing a job that turns out to still exist is
    harmless: the duplicate finds the item finished and is acked/skipped.
    """
    ...
    async def sweep_once(self) -> None:
        ...
        for item in stale_items:
            ...
            if not await claim_for_requeue(self._engine, item.id, stale_after_seconds=self._stale_after_seconds):
                continue
            await self._queue.publish(_job_for(item))
            logger.warning("Item %s was stale; re-published its job (requeue %d)", item.id, item.requeue_count + 1)


def _job_for(item: StaleItem) -> ProcessingJob:
    image = None
    if item.storage_key is not None and item.content_type is not None:
        image = ImageRef(storage_key=item.storage_key, content_type=item.content_type)
